...
<body class="font-serif m-8 bg-gradient-to-br from-gray-900 via-gray-800 to-black">
    <div class="max-w-2xl mx-auto bg-gray-900 p-8 rounded-none border-4 border-double border-gray-600 shadow-[0_0_50px_rgba(0,0,0,0.8)] relative">
        ...
        <h1 class="text-3xl font-bold text-gray-100 mb-6 tracking-wide">Create New Task</h1>
        
        <form action="{{ route('tasks.store') }}" method="POST">
            @csrf
            <div class="mb-6">
                <label for="title" class="block mb-2 font-bold text-gray-300 tracking-wider uppercase">Title *</label>
                <input type="text" id="title" name="title" placeholder="Enter task title" value="{{ old('title') }}" required class="w-full p-3 border-2 border-gray-600 bg-gray-800 text-gray-100 rounded-none focus:outline-none focus:border-gray-500 focus:shadow-[0_0_10px_rgba(100,100,100,0.5)] transition-all">
            </div>
            <div class="mb-6">
                <label for="description" class="block mb-2 font-bold text-gray-300 tracking-wider uppercase">Description</label>
                <textarea id="description" name="description" placeholder="Enter task description" class="w-full p-3 border-2 border-gray-600 bg-gray-800 text-gray-100 rounded-none focus:outline-none focus:border-gray-500 focus:shadow-[0_0_10px_rgba(100,100,100,0.5)] transition-all min-h-[100px]">{{ old('description') }}</textarea>
            </div>
            <div class="mb-6 flex items-center">
                <input type="checkbox" id="is_completed" name="is_completed" value="1" class="mr-3 w-5 h-5 accent-gray-500">
